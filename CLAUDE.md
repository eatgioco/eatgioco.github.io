...
| `gioco-shell.css` | Design system: tokens de cor, tema claro/escuro, sidebar, vidro, `.card`, `.kpi`, `.status`, `.btn-add`, tabelas, e os dois controlos de escolha — `.gio-toggle` (binário) e `.gio-seg` (uma de N, Set/2026) | — |
| `gioco-shell.js` | Sprite de ícones SVG, `giocoIcon()`, sidebar (hover/pin), toggle de tema com persistência, `giocoModal`, e os helpers dos controlos de escolha: `giocoToggleHtml/Set` e `giocoSegHtml/Set/Sync` | — |
...
12. **Controlos de escolha.** Há três, e a escolha entre eles não é de
    gosto — é o que o controlo faz:
    - **`.gio-toggle`** → ligar/desligar UMA propriedade que persiste
      (as flags do A/C, "Incluir caixa no total").
    - **`.gio-seg`** → escolher UMA de N opções mutuamente exclusivas:
      filtros, modos, períodos, vistas, e as perguntas de dois rótulos à
      vista (Saída/Entrada, "Vai haver troco?" Sim/Não).
    - **checkbox nativo** → marcar itens de uma lista (vistos de checklist,
      selecção de linhas) ou um campo dentro de um formulário.

    Os dois primeiros são **obrigatórios, sem excepções**, e zero CSS de
    pílula por página: era o que estava no `.cc-switch` do
    centro-de-controlo e no `.pf-switch` do dashboard (toggles), e no
    `.tipo-toggle`/`.filter-tab` da `caixa.html` (segmentados). É o que o
    `verifica-shell.sh` apanha (ponto 8). Ver as secções "Toggle" e
    "Segmentado" abaixo.
...
- **Estrutura obrigatória:** `<label>` com um `<input type="checkbox">` real
  ...
  `aria-pressed`.
- **O que NÃO é um toggle**: escolha entre opções visíveis, mesmo com duas
  (o par Saída/Entrada e o Sim/Não da `caixa.html` — os dois rótulos à vista
  são a informação) e filtros de vista (Hoje/7 dias/Mês/Tudo) — esses são
  `.gio-seg`, ver a secção a seguir. E, esses sim, não se convertem em
  componente nenhum: disclosure/accordion (`.equip-manage-zona-toggle`
...
**Segmentado (escolher uma de N).** O componente é `.gio-seg` (CSS no
`gioco-shell.css`, helpers `giocoSegHtml()` / `giocoSegSet()` /
`giocoSegSync()` no `gioco-shell.js`), documentado na montra do
`estilo.html`. É **obrigatório** para todo o grupo novo de opções
mutuamente exclusivas: nunca voltar a escrever CSS de pílula segmentada por
página, nem redesenhar o indicador ou as opções do componente (o
`verifica-shell.sh` falha nas duas coisas).

- **ADN partilhado com o `.gio-toggle`**, e é só isto que os faz família:
  `var(--gio-mov)` no movimento do indicador (a mesma curva e duração do
  knob — o token nasceu em Set/2026 desta partilha, e a promoção foi
  validada a zero píxeis de diferença no toggle de tema), a sombra do knob
  no indicador, o raio de 100px, o `:focus-visible` a vermelho e o
  `:disabled` a `.5`.
- **Ao contrário do toggle, ACOMPANHA O TEMA.** O toggle é um objecto por
  cima da página e tem cores literais fixas nos dois temas; o segmentado é
  uma **superfície** da página — um recesso com uma pastilha lá dentro — e
  um recesso claro sobre uma página escura não é um recesso. Daí
  `var(--dough)` no contentor, `var(--white)` no indicador, `var(--ink)` na
  opção activa e `var(--muted)` nas inactivas.
- **O indicador é posicionado em JS**, não em CSS: as opções acompanham o
  texto (padding horizontal constante), nunca uma largura fixa igual para
  todas — "Hoje" e "Últimos 7 dias" numa grelha de colunas iguais deixavam
  espaço morto de um lado e apertavam o outro. Chamar `giocoSegSync(el)`
  depois de QUALQUER `innerHTML` que crie ou revele um grupo, pela mesma
  razão que o `GiocoChart.montar()` existe: sem layout não há medida. Um
  `ResizeObserver` no shell apanha o resto (bloco que deixa de estar
  `hidden`, rotação, fonte a carregar).
- **Uma linha só, sempre.** Sem `flex-wrap`: não cabendo, o contentor faz
  scroll horizontal com snap. Quebrar para uma segunda linha parte a
  leitura do grupo — era o defeito dos filtros da `caixa.html`, com o
  "Tudo" caído sozinho na linha de baixo.
- **Estrutura obrigatória:** `<div role="tablist">` com
  `<button role="tab" data-valor aria-selected tabindex>` e o
  `<span class="gio-seg-ind" aria-hidden="true">` como primeiro filho. O
  estado vive no `aria-selected` — nunca uma classe `.active`. Daí vêm de
  graça o teclado (← → com volta ao início, Home/End) e os leitores de
  ecrã. A página só ouve o clique e lê o `data-valor`: o `aria-selected` e
  o indicador são do componente.
- **Índice −1** (nenhuma opção activa, indicador a `opacity:0`) é estado
  **transitório de formulário**: uma pergunta por responder, ou um campo
  reposto depois de gravar (o Saída/Entrada da `caixa.html` volta a −1 a
  cada registo). **Nunca** válido num filtro, num modo ou numa vista — aí
  há sempre uma escolhida, senão a lista por baixo não tem estado nenhum
  que a explique.
- **Duas medidas:** `.gio-seg` (34px, a de todo o lado) e `.gio-seg--lg`
  (42px, controlo primário ou opções com ícone + texto, que na compacta
  ficam apertadas). Ajustar noutro contexto faz-se pelas variáveis
  (`--gs-h`, `--gs-pad`, `--gs-px`, `--gs-fs`), nunca redeclarando as
  regras. Um ícone numa opção herda a cor do texto e **nunca** substitui o
  rótulo — ao balcão distingue-se num relance sem depender da cor.
- **Variante `.gio-seg--red`** (indicador vermelho, texto branco): só
  quando o segmentado **é** a acção em destaque do bloco. Um filtro em
  vermelho compete com os botões de acção da página e lê-se como alerta.
- **Aderiu até hoje:** só a `caixa.html` (cinco grupos, Set/2026). Ficam
  por converter ~12 grupos noutras 10 páginas (`.tab-btn` de compras,
  contactos, equipa, gestão e receitas; `.vista-btn` do vendas;
  `.pay-pill`/`.notif-filter-chip`/`.filter-chip` do dashboard e da
  tesouraria; `.gioco-tab`/`.pedidos-filter-chip` da loja;
  `.loja-tab`/`.cam-tab` do centro de controlo) e o `.seg` da
  `calendario.html`, que é caso à parte por ter `@media` próprios.
