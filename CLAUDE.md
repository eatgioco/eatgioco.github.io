...
## Equipa (referência)
**Fonte única: `gioco-contexto.js`** (Set/2026). A equipa, os papéis, a entidade legal
e "o Manel é o único decisor" vivem SÓ nesse ficheiro (`GiocoContexto.DADOS` +
`texto()`), que é o bloco de contexto enviado ao Gemini em todos os pedidos do
`gioco-sugestoes.js`. Quando a equipa mudar, edita-se só o array `equipa` desse
ficheiro — não repetir os nomes aqui nem no `gioco-sugestoes.js` (o teste
`testa-sugestoes.js` falha se um apelido da equipa aparecer no módulo de sugestões).
Hoje: Manel (Fundador), Alfredo Giangaspero (Head of Operations), Mattia Pivetta
(Head of Product), Leonor Borges (Head of Brand) — ver o ficheiro para o que cada
um faz.
