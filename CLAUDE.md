...
| `obrigacoes.html` | **Motor de obrigações — Fase 1** (Set/2026): o sítio onde o Manel despeja o que tem de fazer, incluindo projetos de vários passos. Página pessoal: só no conjunto `privada` da nav e no cartão do `mrn-dashboard.html` — nunca no `index.html` nem na nav pública (mesmo padrão de `tarefas.html`/`tesouraria.html`; sem auth, como o resto). Viewport `device-width` + opt-in `shell-mobile` (usa-se ao telemóvel), sem `@media` próprios. **Fluxo de criação (Fase 1.1, Set/2026)** no cartão do topo, duas ações distintas: **Novo projeto** (campo de nome em destaque, ação principal: Enter/«Criar projeto» cria, abre logo a vista do projeto e dispara a sugestão automática de passos) e, por baixo, **Obrigação solta** (secundária: Enter cria; colar várias linhas cria várias — as linhas coladas nunca passam pelo `value` do `<input>`, que descarta `\n`; chip da duração por omissão, 30 min). O select "Sem projeto / + Novo projeto…" SAIU: a captura cria só soltas, e passos de projeto nascem na vista do projeto. **Sugerir passos** (botão na vista de projeto + automático ao criar): `GiocoSugestoes.sugerirPassos(nome)` → lista EDITÁVEL na caixa `#sugBox` (título, duração, select de dependência entre as linhas propostas, ✕ apaga com remapeamento das dependências, «+ Acrescentar linha», «Pedir outra sugestão»); NADA é gravado até «Criar N passos», que cria por ordem sequencial a seguir aos existentes (`sugerida:true`, origem continua `'manual'` — foi o Manel a confirmar), com as dependências mapeadas para ids reais (para trás vão no `set()` inicial; para a frente, um `set()` da folha `dependeDe` no fim) e `ultimoAvancoEm`. Editar um título retoca as `<option>` dos outros selects no sítio, NUNCA re-desenha a lista (o `change` dispara ao mudar de campo e um re-desenho nesse instante trocava o input para onde o foco ia). Falha da API (chave, quota, rede, JSON) → caixa de erro com o `.motivo` e o Decompor manual ao lado: a página funciona na íntegra sem a API. Uma resposta atrasada de um pedido antigo é ignorada (`sugPedido`); mudar de projeto/abrir Decompor fecha a caixa. **Contexto em texto livre** (Set/2026): textarea opcional (placeholder "Escreve tudo o que sabes sobre isto — o que já fizeste, o que te preocupa, o que falta decidir. Opcional.") por baixo do nome em «Novo projeto» (vai no `set()` inicial como `contexto`) e bloco «Contexto» editável na vista de projeto (um `set()` na folha `projetos/{id}/contexto` ao sair do campo — `blur` —, `null` quando fica vazio; estado "guardado dd/mm HH:MM"; o snapshot NÃO pisa o campo enquanto tem foco). «Sugerir passos» grava o que estiver no campo e passa-o em `opts.contexto` — vazio = comportamento só com o nome. **Atualizar passos** (Set/2026, atualização incremental): num projeto que JÁ tem passos o botão principal passa a «Atualizar passos» (`data-modo`; «Sugerir passos» só sem passos). Envia nome + contexto + abertos (id, titulo, ordem, duracaoPrevista, dependeDe) + concluídos (factos consumados) + anulados (não voltar a propor) via `GiocoSugestoes.sugerirAlteracoes` e recebe SÓ alterações `{acrescentar, remover, alterar}`, cada uma com o «porque». Caixa `#atzBox` agrupada por tipo, checkbox por alteração (todas ativas), botão «Aplicar N alterações» que recalcula com o que está marcado; três listas vazias → «Nada a alterar», sem UI de aprovação. Só ao aplicar: alterar = `update()` em `obrigacoes/{id}` só com os campos propostos; remover = NUNCA `remove()`, `update({estado:'anulada', anulado:true, anuladoEm, anuladoPorSugestao:true})`; acrescentar = `criarObrigacao` (`sugerida:true`, `dependeDe:[depoisDe]`) inserido a seguir à âncora (ou no fim) com a ordem 1..n de TODOS os passos recalculada e um `set()` na folha `ordem` só onde muda; depois `ultimoAvancoEm`. Um concluído nunca é tocado — o `normalizarAlteracoes` do módulo descarta ids fora dos abertos e a página volta a filtrar por aberto-do-projeto antes de escrever. A caixa da chave (`#sugChave`) é uma só e é MOVIDA para a caixa que a pediu (`mostrarChave`). Falha da API → erro na caixa, nada gravado, resto da página utilizável. **Local** (Set/2026): campo opcional `local` ∈ loja · computador · rua · telefone · null. O Manel NUNCA é obrigado a escolher — é sempre pré-preenchido pelo modelo e ele corrige quando está errado: (a) a sugestão/atualização de projeto pede `local` no JSON e a lista de aprovação tem um select por linha; (b) a captura de soltas cria IMEDIATAMENTE com `local:null` e só depois, em segundo plano, manda os títulos ao Gemini num único pedido (`classificarLocalEmFundo` → `S.classificarLocal`) e grava cada local com `update({local})` — sem chave, sem rede, quota ou JSON estranho fica null e NÃO se mostra erro nenhum (só `console.warn`); (c) em qualquer linha aberta a etiqueta É um `<select class="local-sel">` compacto (tracejado com "local?" quando não há) e mudar grava `update({local, localManual:true})` — a classificação em fundo nunca escreve por cima de `localManual` nem de um local já preenchido; (e) barra de chips "Local:" na vista principal (Todos por omissão) que filtra a próxima ação por projeto e as soltas com `G.casaLocal` — **uma obrigação sem local aparece SEMPRE**, o que fica de fora conta-se ("· N noutros locais", "(2 de 3)"). **Deteção de posição** (Set/2026, ver nó `mrnLocais`): `<details>` "Locais de referência" no fim da vista principal com «Estou aqui agora» (nome + tipo → `navigator.geolocation.getCurrentPosition` UMA vez → `push().set()` com raio 150), edição inline (um `set()` por folha nome/local/raio) e Apagar (`update({anulado:true, anuladoEm})`, nunca `remove()`). Ao abrir, quando há locais gravados, pede a posição UMA vez (`lerPosicao`, timeout 10 s + rede de segurança 12 s); dentro do raio de um local (`G.localMaisProximo`) aparece o aviso `#geoAviso` "Estás na X — N obrigações podem ser feitas aqui" (N = abertas com esse `local` exato) com o botão que aplica o filtro — NUNCA filtra sem clique. Permissão negada, GPS sem resposta ou fora de todos os raios → nada aparece. Só distância entre coordenadas, sem mapas nem serviços externos. **Vista "Próxima ação"**: por projeto ativo APENAS a obrigação aberta de menor `ordem` cujas dependências estão concluídas (`GiocoObrigacoes.proximaAcao`), nunca a lista toda; soltas em lista própria; projetos com passos abertos mas todos bloqueados por dependências e projetos sem passos abertos têm secções próprias — NUNCA desaparecem. Ordenação (`ordenar`): prazo vencido/hoje → com prazo, mais próximo primeiro → sem prazo, projeto parado há mais tempo primeiro (`ultimoAvancoEm`). Cada linha: título, duração prevista (editável), prazo (editável, `+ prazo`), dias parado, `adiada N×`; ações Concluir (pergunta inline "quanto demorou?", pré-preenchida com a prevista — Enter/um clique aceita → `duracaoReal`), Adiar (só incrementa `adiada`, nada mais muda nesta fase), Anular (`confirm()`), Abrir projeto. Indicadores no topo (abertas · parados > 14 dias · ativos) e secção "Parados há mais de 14 dias". **Vista de projeto** (`?projeto=ID`, hoje o painel lateral): lista completa por ordem, reordenação por drag & drop (antes subir/descer; `set()` por folha `ordem` alterada), dependências por chips (✕ remove) + select "+ depende de…" (`set()` da folha `dependeDe`), selo "próxima ação"/"bloqueada", **Decompor** (textarea, um passo por linha → ordem sequencial a seguir aos existentes, duração por omissão; manual, sem IA), Concluir projeto (só sem passos abertos) e **Anular projeto em cascata** (Set/2026): a confirmação diz «Anular o projeto X e os N passos abertos?», e ao aceitar os passos ABERTOS são anulados com `anuladoPorProjeto:true` e o MESMO `anuladoEm` do projeto — primeiro os passos, um `update()` por passo, o projeto por último (se um passo falhar o projeto continua ativo e repete-se); os concluídos ficam intactos. Nenhum passo de projeto anulado reaparece como solto. Um passo aberto de projeto anulado (dados antigos) o motor continua a mostrar como solta — nunca esconde — e corrige-se com `scripts/corrige-passos-projetos-anulados.js` (só relatório por omissão, `--aplicar` grava). **UX ao estilo Notion (Set/2026, cinco partes):** (1) **Edição inline** — o título de cada linha aberta (`.t[data-edit="titulo"]`), o nome do projeto no painel (`#projNome[data-edit="projNome"]`) e a linha nova são `contenteditable` no sítio, mesma fonte, sem salto: clicar entra em edição, Enter/blur gravam (`update({titulo})` em `obrigacoes/{id}`, `update({nome})` em `projetos/{id}`), Escape cancela, confirmação = contorno verde a desvanecer (`piscarGuardado`), nunca alert. Enquanto há edição (`edicaoAtiva`) o `render()` fica suspenso (`renderPendente`) — o snapshot chega a cada escrita e destruiria o nó com foco; o Enter/Escape da edição levam `stopPropagation` para não voltarem a entrar em edição num nó já desligado. (2) **Linha viva** — pega `⋮⋮` (`.l-grip`) à esquerda e `⋯` (`.l-mais`) no fim, só em hover (sempre visíveis com `body.shell-touch`); `+` fino entre linhas (`.l-inserir`) insere a seguir uma linha TEMPORÁRIA (`novoTemp`, `[data-temp]`) que só existe no DOM até Enter com texto — vazia ou Esc descarta sem escrever nada; num projeto entra com `criarPassoApos` (ordem = posição, seguintes renumerados folha a folha). O menu `⋯` (`#menuPop`, um só, reposicionado; `position:fixed` dentro do painel) tem duração prevista (input inline), prazo, adiar, duplicar (`criarObrigacao` com o mesmo título/duração/local/prazo, a seguir ao original) e anular (`confirm()` + flag, nunca `remove()`); Adiar e Anular saíram dos botões visíveis, só «✓ Concluir» e «Abrir projeto» ficam à vista. (3) **Drag & drop** pela pega, só dentro do MESMO projeto (contentor `[data-dnd-projeto]`: lista do painel e coluna do projeto no Quadro): HTML5 drag no rato, pointer events com long-press de 350 ms ao toque (ghost + marca de largar antes/depois); largar = `renumerar(pid, ids)` → `set()` na folha `ordem` 1..n só onde muda, que é também o backfill de passos sem ordem numérica. (4) **Painel lateral** (`<aside id="painel">`, `width:min(600px,100vw)`, ecrã inteiro com `shell-touch`): substituiu a antiga vista de projeto a página inteira; abre ao clicar no nome do projeto (`.proj`, `data-acao="abrir-projeto"`) ou no deep-link `?projeto=`; nome editável inline, contexto com auto-save (blur E 1,2 s depois de parar de escrever), lista completa de passos com as mecânicas 1–3, Sugerir/Atualizar/Decompor/Concluir/Anular como antes; fecha com Esc, clique no fundo ou ×. A lista principal não re-desenha ao abrir nem perde scroll. (5) **Toolbar e Quadro** — toolbar numa linha (scrollável ao toque, `+` sticky à direita): chip Lista/Quadro (`localStorage 'obrigacoes.vista'`, Lista por omissão), chip «Agrupado por: projeto / local / semana» (`'obrigacoes.grupo'`), ícones fantasma procurar (filtra por título e conta «N fora da procura»), filtrar (mostra/esconde a barra de chips de local) e ordenar (prazo/antiguidade = regra do motor · título A–Z · mais recentes), e o `+` vermelho com menu (novo projeto / nova solta / novo passo no projeto aberto). Em Lista, agrupar por local/semana desenha uma secção por grupo com TODAS as abertas (`#listaAgrupada`); por projeto é a vista de sempre. O **Quadro** (`gruposDe()` → colunas `.q-col` com `--q-cor` = `--grupo-N`, fundo a 6 %, raio 16, pill com ponto + nome + contagem, `⋯` e `+` discretos, cartões `.q-card` com título + subtítulo, «+ Novo» na cor do grupo dentro da coluna): 280px no desktop, 85vw ao toque para a coluna seguinte espreitar, `scroll-snap`. Grupos: projeto = um por projeto ativo (mesmo sem passos) + «Sem projeto»; local = os 4 locais + «Sem local»; semana = Vencidas / Esta semana / Próxima semana / Mais tarde / Sem prazo (o «+ Novo» de Esta/Próxima pré-preenche o prazo). **Nenhum agrupamento, filtro, vista ou ordenação esconde uma aberta sem contar** — é a regra do motor. Cada secção tem uma linha de instrução (`.sec-sub`) e ao fazer scroll (> 140px) aparece a barra fina `#topoFino` com o título e o nº de abertas. Não há `.gio-toggle` na página por não existir nenhum controlo binário. **Nomenclatura:** dentro de um projeto são «passos» (textos, confirmações e metas no masculino: «anulado · com o projeto», «bloqueado»); «obrigações» é o conceito geral e o título da página; as avulsas são «obrigações soltas». Fechadas ficam todas em `<details>` "Concluídas e anuladas (N)", sem limite. Toda a lógica pura vive no `gioco-obrigacoes.js`. FORA desta fase: moldura do dia, janela de tempo, recorrência, alimentadores automáticos, Outlook, ligação a pagamentos/faturas/contagens | Só Manel |
...
obrigacoes            — MOTOR DE OBRIGAÇÕES, Fase 1 (Set/2026). Escrito SÓ pela
                         ...
                         anuladoPorProjeto? (true quando o passo foi anulado em
                         cascata ao anular o projeto — Set/2026; anuladoEm é o MESMO
                         do projeto; ausente nas outras anulações) }. O RTDB descarta nulls:
                         ...
                         (prazo e dependeDe a null quando se limpam — a única escrita
                         de null). NUNCA set()/update() no nó pai obrigacoes/.
projetos              — projetos/{pushId} = { nome, estado ('ativo' | 'concluido' |
                         ...
                         vai ao Gemini em cada «Sugerir passos»; ausente = null) }.
                         ...
                         projeto escreve-se por último. NUNCA remove().
