...
| `reconciliacao.html` | **Reconciliação** (Set/2026), privada, em DOIS CARTÕES (redesenho de 10 Set/2026): o ÚNICO ponto de entrada é o cartão no `mrn-dashboard.html` (não está no `index.html` nem em `GIOCO_NAV_CONJUNTOS`; o back-link para o dashboard está na lista de excepções do `verifica-shell.sh`). **NUNCA escreve no Firebase.** Nomes visíveis em toda a interface, sem excepção: **POS, TPA ABANCA, TPA VERIFONE, CAIXA** — `OU`/`CD`/`NU` do SAF-T e `INTERCARD`/`FECHO TPA` do banco são chaves internas (`MEIOS` no topo). **Cartão 1 "Onde está o erro" — SEM uploads**, só com o Firebase, lido com `once()`: `vendasDiario/{mês}` de TODOS os meses (`porPagamento`), `contasBancarias/{abanca,revolut}/movimentos` (só CRDT), `caixaMovimentos`, `cofreMovimentos`, `caixaContagens`. Topo: total "por cobrar / em falta", tiles por tipo (dias verificados, com erro, €, "cobrado a mais"), lista dos dias com erro (os que batem escondidos, botão "Mostrar os N que batem"). **Triagem de cartão** (`triagemCartao`), calibrada nos dados reais: (a) **TPA ABANCA** — o crédito `FECHO TPA` cai no PRÓPRIO dia (`booking_date` = dia) e vale bruto do terminal − TSC/RSC desse dia, confirmado ao cêntimo no extrato 27/08–05/09; a taxa varia com o mix de cartões (0,532–0,829 %), por isso o bruto é reconstruído como INTERVALO `[crédito/(1−0,50 %), crédito/(1−0,85 %)]` e só acusa quando o POS fica fora dele (POS acima → "por cobrar"; abaixo → "cobrado a mais" = gorjeta, informativo, fora do total). NÃO há atraso de 1–5 dias nem lote: a premissa inicial estava errada para este terminal. (b) **TPA VERIFONE** — créditos `INTERCARD` BRUTOS (comissão debitada à parte), um por dia de POS, em 0–5 dias (sex/sáb/dom caem à segunda); 73 dos 87 dias batem ao cêntimo com `vendasDiario`, logo limiar 0. Algoritmo: passagem exacta dia a dia (1 crédito ou o grupo de créditos do mesmo dia na janela) → resíduo em COMPONENTES ligadas pela janela; componente com tantos dias como créditos emparelha 1:1 por ordem (permutando créditos do mesmo dia para minimizar a soma das diferenças) e dois dias seguidos com diferenças que se anulam são a troca de dia à meia-noite (01+02/08) e somam-se num item; caso contrário compara a soma. Sem crédito e `dia+janela > último dia do banco` → "aguarda banco"; dia anterior ao início do banco sem match exacto → "não testável" (fora das componentes); crédito sem dia → "crédito sem venda". O início do banco é o da CONTA onde caem os créditos do terminal. **Triagem de caixa** (`intervalosCaixa`, a mesma função para as duas fontes): contagens físicas do Fecho de Caixa do ZSBMS (coluna Início; hora de abertura da Listagem de Sessões quando carregada, senão 00:00 do dia, marcado "aproximado") OU, sem ficheiro, `caixaContagens.total` + `caixaMovimentos`; entre duas contagens `real = contagem seguinte − contagem + saídas POS − entradas POS + saídas OS − entradas OS` (saída acertada = valor − valorDevolvido; entrada no cofre "da caixa" sem par em `caixaMovimentos` conta como saída) contra o numerário do POS no intervalo: SAF-T à hora quando cobre todos os dias tocados, senão `vendasDiario` por dia de calendário atribuído ao intervalo que contém as 17:30; tolerância 0,05 €. Sem SAF-T, uma contagem entre 12:15 e 22:45 torna o intervalo "não testável"; nos 15 min à abertura/fecho passa como aproximado. **Cartão 2 "Investigar"**: QUATRO inputs com selecção múltipla e acumulação (SAF-T · extrato TPA ABANCA · extrato TPA VERIFONE · Fecho de Caixa, que aceita também a **Listagem de Sessões** — o tipo é detectado pelo cabeçalho em `parseZsbms`), cada ficheiro com origem e botão "remover"; `docs`/`terminal`/`fecho`/`sessoes`/coberturas RECONSTRUÍDOS da lista a cada mudança (`reconstruir()`); dedupe dentro de cada origem. Cobertura de um extrato = período declarado ∩ [1.ª transação, hoje]. `parseSessoesDoc` lê pelo CABEÇALHO (abertura/fecho, ou data+hora em colunas separadas; `dd-mm-aaaa hh:mm`, `dd/mm/aaaa hh:mm` ou ISO) porque o export real ainda não foi visto — se falhar, o erro lista os cabeçalhos encontrados. Mesmo casamento linha a linha de sempre (janelas 2/15/60 min, "valor alterado no terminal", conta dividida; ver constantes no topo), depois o painel **"Vendas sem cobrança"** com a cascata H1 cobrado no outro terminal (transação livre da outra origem, mesmo dia, mesmos cêntimos, ±60 min) → H2 pago em dinheiro, agora POR SESSÃO: cada documento vai para o intervalo entre contagens que contém a sua hora e o excedente desse intervalo (`intervalosCaixa`) explica-o quando bate, à tolerância, com UM ÚNICO subconjunto (até 16 docs) → H3 sem entrada de dinheiro (destaque) → não testável (fora dos totais, contagem sempre visível). Por baixo, a lista de dias (filtro "Todos os meses" por omissão) e o detalhe: sem par e ambíguas, valor alterado, casadas, totais, recusadas/canceladas, anulados, outros meios. Teste de referência 04/09/2026 (cartão 2): 4 docs sem cobrança (3970, 3996, 4006, 4009 = 115,40 €), H1 negativa; no cartão 1 o mesmo dia dá TPA ABANCA −105,11 € (= −115,40 + ~8,4 € de gorjeta + reconstrução da comissão). Ficheiros de teste em `_teste/` (ignorado pelo git) |
