...
- **Excepção à excepção: a `calendario.html` tem `@media` próprios** (Set/2026; desde
  ...
  `@media (max-width:700px)`, onde as grelhas do calendário de conteúdo — mês e semana —
  dão lugar à MESMA lista por dia e a tabela da vista Lista larga duas colunas: sete
  colunas de dias não cabem a 375px, e é lá que vive o caminho sem arrasto). Um calendário não se reorganiza só com `shell-touch`: ao
