...
| `loja-app-teste.html` | loja-app-teste.html — app do staff com o novo design, em teste de terreno; dados de demonstração, sem Firebase, fora da navegação; decisão de avançar pendente do Manel. Página autónoma (CSS/JS próprios, sem `gioco-shell`; excepção no `verifica-shell.sh`). Corre em hora real; barra de simulação escondida, abre com 5 toques na faixa "Versão de teste" ou no rodapé | Equipa (teste) |
...
| `gioco-caixa.js` | Motor partilhado do **dinheiro físico** (`GiocoCaixa`), funções puras sobre os nós em memória, SÓ leitura: `ultimaContagem(contagens)`, `saldoVivoCaixa(contagens, movs)` (null sem contagem — nunca 0), `saldoCofre(cofreMovs)` (cumulativo), `dayKeyLocal(iso)` e, desde 19 Set/2026, **`efeitoMovimentoCents(m)`** — a ÚNICA definição do efeito de um `caixaMovimento` no saldo (entrada +valor; saída `acertado` −(valor − valorDevolvido); qualquer outra saída −valor), em cêntimos inteiros, usada pelo `saldoVivoCaixa` E por **`reconciliarNumerario(contagens, movs, nuPorDia, posPorDia)`** → `{ linhas:[{dias, ancoraDe, ancoraAte, nu, entradasOs, saidasOs, movimentos, saidasPos|null, entradasPos|null, semPos, semNu, esperado, contado, delta, deltaCents, estado, motivoDiferenca, chave}], contagensInvalidas, movimentosInvalidos }` — o check diário do numerário da `reconciliacao.html` (equação, estados e regra "sem tolerância, aritmética em cêntimos" no cabeçalho do ficheiro e na entrada dessa página). `posPorDia` é opcional (dia sem dados POS → `saidasPos:null`). Extraído da `caixa.html` em Set/2026 sem alterar um cêntimo (o `saldoVivoCaixa` passou a somar em cêntimos e dá o mesmo resultado — regressão em `scripts/testa-caixa.js`); usado pela `caixa.html` (cards Caixa/Cofre), pelo `mrn-dashboard.html`/`dashboard.html` (Posição Financeira) e pela `reconciliacao.html`. Também exporta `module.exports` para Node. Nunca reimplementar por página | — |
...
| `reconciliacao.html` | **Reconciliação** (reescrita completa em 10 Set/2026), privada, em DOIS CARTÕES: o ÚNICO ponto de entrada é o cartão no `mrn-dashboard.html` (não está no `index.html` nem em `GIOCO_NAV_CONJUNTOS`; o back-link para o dashboard está na lista de excepções do `verifica-shell.sh`). **Deixou de ser só-leitura**: escreve descartes (`reconciliacaoDescartes/`) e resoluções (`reconciliacaoResolvidos/`) — ver esses nós abaixo; os ficheiros carregados continuam só em memória, nunca gravados. Nomes visíveis: **POS, TPA ABANCA, TPA VERIFONE, CAIXA (numerário)** — `OU`/`CD`/`NU` do SAF-T e `INTERCARD`/`FECHO TPA` do banco são chaves internas (`MEIOS` + `MEIO_NU` no topo). **CHECK DIÁRIO DO NUMERÁRIO** (19 Set/2026, decisão do Manel que REVERTE a remoção anterior — o `esperado` gravado em `caixaContagens` é última contagem + `caixaMovimentos` SEM as vendas em numerário do POS, logo a diferença da contagem da manhã era sobretudo a venda de ontem e ninguém reconciliava o numerário): a `caixa.html` fica EXACTAMENTE como está; o Cartão A ganha o terceiro mecanismo **CAIXA (numerário)**, com as mesmas colunas (Faturado POS = esperado, Creditado Banco = contado), o mesmo padrão visual e o mesmo Descartar, calculado por `GiocoCaixa.reconciliarNumerario` (`gioco-caixa.js` — a página só liga dados: `caixaContagens` e `caixaMovimentos` lidos com `once()`, o NU de `vendasDiario/{mês}/{dia}/porPagamento/NU.bruto` já em `vendasMes`, e `posPorDia` do **Fecho de Caixa ZS** carregado no Cartão B, que deixou de ser "só registo" e passa a alimentar este check, sempre só em memória). A EQUAÇÃO: âncora do dia D = a PRIMEIRA contagem válida do dia LOCAL D (`dayKeyLocal`, nunca a fatia UTC; contagens a meio do dia são ignoradas); janela = (âncora D, âncora seguinte]; esperado = total da âncora D + NU dos dias de operação cobertos + entradas − saídas de `caixaMovimentos` na janela (o MESMO efeito por movimento do `saldoVivoCaixa`, via `efeitoMovimentoCents`) − saídas POS (coluna Saídas do Fecho ZS) + entradas POS; Δ = total da âncora seguinte − esperado. Manhã sem contagem → uma linha só com os dias agregados ("12/09 → 13/09"), NU e saídas POS somados — nunca se inventa uma âncora. **SEM TOLERÂNCIA NENHUMA** (mesma regra da contagem na `caixa.html`, nem 0,01 €; não existe constante de tolerância): TODA a aritmética é em cêntimos inteiros (`Math.round(valor × 100)` em cada parcela ANTES de somar, o Fecho ZS com três casas arredondado ao cêntimo à entrada) e só se converte para euros na apresentação — Δ = 0 cêntimos é o ÚNICO OK. Estados: **Em aberto** (sem âncora seguinte — hoje nunca confirma hoje; sem botão descartar), **Provisório** (há Δ mas falta o Fecho ZS de algum dia da janela — nota "sem saídas do POS" — ou o NU importado desse dia), **OK** (Δ exactamente 0,00 €, verde só aqui), **Falta** (Δ < 0), **Sobra** (Δ > 0). "Não Reconciliados" inclui A Menos dos TPA E Falta E Sobra do numerário (a sobra pode explicar vendas sem cobrança); Provisório e Em aberto não entram nos totais; quarto total no topo **"Falta em caixa"** (soma dos Δ das linhas Falta, fora as descartadas). Δ ≠ 0 é sempre um evento real com causa, nunca arredondamento — a UI NUNCA diz que cêntimos são normais nem sugere descartar por rotina; o Descartar é para o caso investigado, com motivo escrito, pela chave `{dias.join('_')}~NU` em `reconciliacaoDescartes` (a única escrita deste check — nada em `caixaContagens`/`caixaMovimentos`). Cada linha expande (clique na linha, `nuAberto`) e mostra a conta por extenso: âncora D (hora, pessoa), NU, cada movimento do OS na janela (hora, motivo, valor com sinal), saídas/entradas POS, esperado, âncora seguinte (hora, pessoa), Δ e o "Porquê?" gravado nessa contagem; tudo `.num`. Contagens/movimentos com `dataHora` inválida ficam fora e a página avisa (`.rec-nu-aviso`). O Cartão B (dias a investigar, talão a talão) continua só com os dois TPA. Testes: `scripts/testa-caixa.js`; `scripts/fase1-numerario.js` imprime a tabela real por REST (só leitura). **Cartão A "Reconciliações Base" — sem ficheiros**, só `vendasDiario/{mês}` (`porPagamento`) e `contasBancarias/{abanca,revolut}/movimentos` (só CRDT), lidos com `once()`. `atribuirCreditos(meio)` faz a MESMA atribuição crédito↔dia de sempre (greedy exacto ao cêntimo → componentes ligadas pela janela do banco → `emparelhar` 1:1 quando dias e créditos batem em número, minimizando a diferença) mas o resultado mostrado é sempre o Δ CRU: Faturado POS − Creditado Banco, sem repor comissão nem intervalos. Δ% = Δ/Faturado POS. **Estado** (`classificarItemA`, banda `BANDA_OK` por TPA, documentada no cabeçalho do `<script>`): OK dentro da banda calibrada na comissão observada (TPA ABANCA −0,3 % a 1,2 %; TPA VERIFONE −0,3 % a 0,3 %, crédito bruto); acima → A Menos; abaixo → A Mais; quando a atribuição não resolve 1:1 (componente com vários dias E vários créditos sem correspondência clara) → Ambíguo, seja qual for o Δ%. Dias ainda dentro da janela de espera do banco ou anteriores ao início do extrato ficam "não verificável" e não aparecem na lista. Totais: A menos (total), A menos TPA ABANCA, A menos TPA VERIFONE — sempre excluindo as linhas descartadas. Filtro único (`filtroA`): **Não Reconciliados** (só A Menos, por omissão) · Todos os meses · cada mês. **Descartar**: botão por linha → formulário inline (motivo obrigatório + autor) → `reconciliacaoDescartes/{chave}.set()`, chave = `{dias.join('_')}~{mec}` (ex. `2026-09-04~OU`); as descartadas saem dos totais e da lista, com contador "Ver N descartados" e botão "reactivar" (`update({anulado:true, anuladoEm})` — nunca `remove()`; descartar de novo depois de reactivada faz outro `set()` completo, que limpa a flag). **Cartão B "Não Reconciliados Detalhe"**: quatro uploads (SAF-T · TPA ABANCA · TPA VERIFONE · Fecho de Caixa ZS, que aceita também a Listagem de Sessões — tipo detectado pelo cabeçalho em `parseZsbms`) com selecção múltipla, acumulação (`reconstruir()`, dedupe por origem) e, por linha, as datas identificadas como A Menos no cartão A que ainda não têm ficheiro dessa origem. O Fecho de Caixa fica só como registo — não entra no casamento (a reconciliação de caixa saiu daqui). SAF-T (`parseSaft`) extrai agora também os artigos de cada `<Invoice>`: `linhas:[{descricao, quantidade}]` de `<Line><ProductDescription>/<Quantity>`. Lista: só os dias A Menos (não descartados) do cartão A, expansível, com as vendas sem cobrança desse dia (Data, Hora, Talão, Valor, Artigos). **Algoritmo do casamento** (`casarDiaMeio`, por dia e por TPA, pela ordem exacta): 1) `casarGlobal` — TODOS os pares (documento, transação) com o mesmo valor em ±60 min, ordenados por distância temporal crescente, atribuídos saltando os já usados (nunca greedy por documento em ordem cronológica); 2) `casarAlterado` — sobre o resíduo do passo 1, ±3 min, valor diferente, só quando sobra exactamente 1 candidato de cada lado (gorjeta); 3) `casarDividida` — sobre o resíduo do passo 1 (não do 2), ±5 min, N transações cuja soma bate com um documento, tolerância 5 cêntimos por parcela (conta dividida); 4) o que sobra dos três passos, confirmado que também não bate com o mesmo valor em ±60 min numa transação livre do OUTRO terminal (`vendasSemCobrancaDoDia`) → venda sem cobrança. **Resolver**: botão por venda → formulário inline (razão obrigatória + autor) → `reconciliacaoResolvidos/{chave}.set()`, chave = `InvoiceNo` com `/` trocado por `~` (ex. `FS 1A2601~3970`); resolvidas ficam visíveis esbatidas com a razão à vista, fora do total "por resolver". Anular seria por flag, nunca `remove()` (sem botão de anular na UI ainda). Ficheiros de teste em `_teste/` (ignorado pelo git) | Só Manel |
...
caixaContagens        — contagens físicas de caixa por denominação (Set/2026), escrito
                         ...
                         ATENÇÃO (19 Set/2026): o `esperado` gravado aqui NÃO inclui as
                         vendas em numerário do POS (que só existem no OS depois do import
                         do SAF-T) — a `diferenca` da contagem da manhã é sobretudo a venda
                         de ontem, e NÃO é o check do numerário. Esse check vive na
                         reconciliacao.html (Cartão A, CAIXA (numerário)) e lê este nó só
                         em leitura (`once()`): a PRIMEIRA contagem válida de cada dia LOCAL
                         é a âncora do dia; as contagens a meio do dia são ignoradas nele.
                         Nada nesse check escreve aqui
