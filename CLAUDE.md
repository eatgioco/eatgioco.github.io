...
| `reconciliacao.html` | **Reconciliação POS ↔ TPA** (Set/2026), privada: o ÚNICO ponto de entrada é o cartão no `mrn-dashboard.html` (não está no `index.html` nem em `GIOCO_NAV_CONJUNTOS`; o back-link para o dashboard está na lista de excepções do `verifica-shell.sh`). Compara, transação a transação, os documentos do SAF-T (ZIP do ZoneSoft lido no browser com JSZip 3.10.1, mesmo padrão do `vendas.html` — NÃO altera a importação) contra o extrato CSV do TPA. Tudo em memória, perde-se ao recarregar (dito na interface); **NUNCA escreve no Firebase** — lê só `vendasDiario/{mês}` (`porPagamento`) para o modo degradado. Meios: `OU` = TPA ABANCA (código literal no `PaymentMechanism`, confirmado num SAF-T real) e `CD` = **TPA INTERCARD/Verifone** (confirmado pelo extrato: 06/09 soma 182,90 € de ambos os lados; rotulado "INTERCARD" só quando há extrato Verifone carregado, "CD" sem ele). Extrato Verifone (`parseIntercard`, opção "INTERCARD (Verifone) — CD"): CSV UTF-8 com BOM, vírgula, TODOS os campos entre aspas (parser RFC 4180 próprio, `parseCsvAspas`), decimal com PONTO, cabeçalho na 1.ª linha, 100+ colunas usadas SÓ por nome (`created_at_date`/`created_at_time` = hora LOCAL, nunca convertida para UTC — o `SystemEntryDate` do SAF-T também é local; `created_at_timezone` ≠ `+01:00` gera aviso no ficheiro, não conversão; `Curr.amount`; `status`; `Product`; `payment_method`; `masked_card_number`; `authorisation_code`; `_id` = chave de deduplicação, NUNCA para casar com o POS; `batch_id` mostrado como "lote …" quando `settlement_date` vem vazio, nunca inventar data). SEM comissão por transação (`payment_processing_fee_amount` vazia; o Verifone cobra por débito mensal): `comissao:null`, coluna "—", líquido = bruto — nunca zeros que pareçam comissão. Estados: `SALE SETTLED` e `SALE AUTHORISED` entram no casamento e nas somas; `SALE DECLINED`, `CANCEL DECLINED`, `CANCEL REJECTED` ficam fora de tudo mas aparecem na lista "Recusadas e canceladas no terminal" do dia; qualquer outro status é "estado não reconhecido", fora das somas e destacado, nunca ignorado em silêncio (`INTERCARD_STATUS_VALIDO`/`_EXCLUIDO`). Cobertura de dias = do primeiro ao último `created_at_date` do ficheiro. Do SAF-T guarda TODOS os documentos e todas as linhas `Payment` (casa pelo `PaymentAmount`, nunca pelo `GrossTotal`; `InvoiceStatus = A` fora das somas, listados à parte; hora = `SystemEntryDate`). CSV ABANCA: preâmbulo até à linha `ESTABELECIMENTO;NOME ESTABELECIMENTO` (a linha `PERÍODO:` dá a cobertura de dias), colunas por nome, vírgula decimal, `Nº OPERAÇÃO` só para deduplicar entre uploads, `DATA DA OPERAÇÃO` é a data do check, `DATA DE LIQUIDAÇÃO` só informativa, montante negativo = devolução com sinal. Casamento por VALOR + HORA em passagens `JANELAS_MINUTOS = [2, 15, 60]` (constante no topo; a de 2 min entrou porque dois clientes com o mesmo artigo na mesma hora davam ambiguidade falsa): casa só quem tem o valor único de ambos os lados na janela; sobra com candidatos exactos = ambíguo (todos listados, nenhum escolhido). Às órfãs sem candidato exacto de nenhum lado aplica-se ainda o estado **"valor alterado no terminal"** (`detectarAlterados`, `JANELA_ALTERADO_MINUTOS = 2`): POS e terminal a ±2 min com valores diferentes = a mesma venda com gorjeta arredondada ao euro no terminal (acontece todos os dias); e a **conta dividida** = N transações a ±2 min cuja soma bate com UM documento (`TOLERANCIA_DIVISAO_CENTS_POR_PARCELA = 5`, `MAX_PARCELAS_DIVISAO = 6`), rotulada "dividida em N". Primeiro a dividida, depois o 1:1, ambos só quando a leitura é única. Estes pares contam como CASADOS (não são órfãs) e a soma das diferenças é a linha "Cobrado a mais do que faturado: X €" no topo do dia — o número que o Manel quer ver todos os dias (07/09: 2,20 + 1,70 + 0,07 = 3,97 €). O que sobrar sem candidato = só no POS / só no terminal. Zona 1 = lista de dias com filtro de mês cuja primeira opção e omissão é **"Todos os meses"** (lista inteira do período, ordenada por dia mais recente primeiro atravessando meses, nunca paginada nem cortada — em "Todos" carrega `vendasDiario/{mês}` de todos os meses desde `PRIMEIRO_MES`) (verde bate, vermelho com o Nº DE LINHAS SEM PAR, cinzento sem ficheiros; "extrato sem linhas" quando o período do CSV cobre o dia mas não há linha dele); Zona 2 = detalhe sempre visível: sem par e ambíguas primeiro, casadas lado a lado, totais, anulados, e os documentos dos outros meios (NU…) numa lista à parte. Sem tolerâncias. CSS com prefixo `rec-`. **Ficheiros em baldes** (10 Set/2026): cada ficheiro carregado guarda o que leu (`res`) e `docs`/`terminal`/`fecho`/coberturas são RECONSTRUÍDOS da lista (`reconstruir()`) a cada carregamento ou remoção — os extratos das duas origens coexistem, os SAF-T acumulam e cada ficheiro tem botão "remover" sem perder os outros. **Terceiro input: Fecho de Caixa do ZSBMS** (`parseFechoCaixa`: .xls que é HTML ISO-8859-1, tabela `Data|Inicio|Entradas|Saidas|Movimento|Fecho`, três decimais, vírgula decimal, espaço nos milhares, linha `Total` ignorada; só a coluna Início é contagem real, o Fecho é aritmética). **Painel "Vendas sem cobrança"** no topo, sempre visível, sobre TODOS os dias carregados: cada documento "só no POS" contra o extrato do seu meio passa pela cascata H1 cobrado no outro terminal (transação LIVRE da outra origem — `termSemPar` do casamento dela —, mesmo dia, mesmos cêntimos, ±60 min, a mais próxima; uma transação explica um só documento) → H2 pago em dinheiro, testada AO DIA: dinheiro real = abertura seguinte (linha seguinte do Fecho, não dia+1) − abertura do dia + saídas − entradas, contra o numerário do POS (NU do SAF-T, senão `vendasDiario`); excedente > 0 só explica documentos quando bate (tolerância `TOLERANCIA_DINHEIRO` = 0,05 €) com UM ÚNICO subconjunto deles (até 16; acima só o total) → H3 sem entrada de dinheiro = o resto, em destaque. "Não testável" (fora dos totais): dias sem SAF-T (colapsados por intervalo), dia/meio sem extrato próprio OU com extrato cujo período cobre o dia mas SEM NENHUMA linha dele (exportado antes do fecho — senão tudo ficava "só no POS"), documentos sem extrato da outra origem (H1) ou sem Fecho de Caixa / sem abertura seguinte (H2). Cabeçalho diz "N dias verificados a sério de M". Teste de referência: 04/09/2026 → 4 docs (3970, 3996, 4006, 4009 = 115,40 €), H1 negativa, H2 negativa (numerário POS 72,50 €, real 10,20 €, excedente −62,30 €) | Só Manel |
