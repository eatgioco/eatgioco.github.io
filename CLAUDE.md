...
8. **Ícones.** Sprite local do `gioco-shell.js` (`<use href="#i-…">`), zero CDN.
   O sprite tem 50 símbolos (Set/2026: entrou `sparkles`, para o «Sugerir passos» da `obrigacoes.html`; antes `video`, para a videochamada nos eventos do Outlook da `calendario.html`; antes disso `chevron-left`, `calendar` e
...
## Equipa (referência)
- **Manel** — Fundador e único decisor.

A equipa NÃO está descrita em lado nenhum do código (decisão de 12/09/2026): o
`gioco-contexto.js` deixou de ter a lista de pessoas, papéis e funções — desatualizava-se
e contexto errado é pior que contexto ausente. O modelo do `gioco-sugestoes.js` descreve
terceiros pelo papel genérico e só usa um nome quando o Manel o escreve no contexto de um
projeto. O `testa-sugestoes.js` falha se um nome próprio de pessoa aparecer no
`gioco-contexto.js` ou no `gioco-sugestoes.js`. Não voltar a pôr a equipa aqui nem lá.
