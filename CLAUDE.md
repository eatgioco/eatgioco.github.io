...
- **Excepção à excepção: a `calendario.html` tem `@media` próprios** (Set/2026; desde
  13 Set/2026 a `loja-sao-bento.html` tem também UM `@media (min-width:900px)`, só para o
  `.topo-grid` — ver a tabela de ficheiros; e a `social.html` tem UM
  `@media (max-width:700px)` — o MESMO limite que o JS dela lê em `MQ_MOBILE`, e os dois
  têm de mudar juntos — onde a vista Mês do calendário de conteúdo passa a células
  compactas com painel do dia, a Semana dá lugar à lista por dia, a tabela da Lista larga
  duas colunas, os filtros recolhem atrás de um botão e a legenda fecha). Um calendário não se reorganiza só com `shell-touch`: ao
  telemóvel segue o esqueleto de navegação do Outlook mobile, com o design system GIOCO
  (2.ª versão, Set/2026 — a 1.ª, com pontos coloridos nas células do mês, foi rejeitada
  por não se ler o que há em cada dia). Tudo vive num só bloco
  `@media (max-width: 767px)` no fim do `<style>` da página, e **só nesse bloco** — o
  `gioco-shell.css` e o `gioco-eventos.css` não foram tocados. Regras: o limite é de
  LARGURA (767px) e o JS lê o MESMO limite do `matchMedia('(max-width: 767px)')` a cada
  chamada de `ehMobile()` (constante `MQ_MOBILE`, sem o número repetido em mais lado
  nenhum) e volta a desenhar no `change` (rotação do ecrã) — os dois têm de mudar juntos;
  acima de 768px a página é byte a byte o que era (verificado com diff de píxeis das
  ...
  **cópia estrutural do Outlook mobile** (`.mob-bar`, sticky): linha 1 = mês + ano com
  ...
  os mesmos handlers, mas o `.cal-bar` é `.glass-light` (backdrop-filter) e prende lá
  dentro qualquer `position:fixed` filho. Os toques nos dias da faixa resolvem-se no
  `pointerup` (com o ponteiro capturado, o `click` é retargetado para a faixa e o botão
  do dia nunca era encontrado); o `click` fica só para o teclado. Nenhuma leitura ou
  escrita no Firebase mudou.
